...
export class SocialWorkflow extends WorkflowEngine {
  ...
  get socialConfig(): SocialConfig {
    return this.config as SocialConfig;
  }
  ...
  async buildConfigFromAnswers(answers: Record<string, string>): Promise<SocialConfig> {
  ...
  private async searchStep(ctx: StepContext): Promise<StepResult> {
  ...
  private async filterStep(ctx: StepContext): Promise<StepResult> {
  ...
  /** Step 3: Score leads */
  private async scoreStep(ctx: StepContext): Promise<StepResult> {
  ...
  private async draftStep(ctx: StepContext): Promise<StepResult> {
  ...
  private async previewStep(ctx: StepContext): Promise<StepResult> {
  ...
  private async postStep(ctx: StepContext): Promise<StepResult> {
    const drafts = (ctx.data.drafts ?? []) as DraftReply[];
    let posted = 0;
  ...
  private async trackStep(ctx: StepContext): Promise<StepResult> {
    const drafts = (ctx.data.drafts ?? []) as DraftReply[];
    const totalCost = drafts.reduce((sum, d) => sum + d.estimatedCost, 0);
